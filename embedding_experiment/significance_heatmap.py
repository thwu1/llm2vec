...
plt.xticks(ticks=np.arange(size), labels=reversed(sorted_benchmarks), rotation=90)
plt.ylabel('Value')
plt.title('Row Sum of Contribution Matrix $C$')
...
plt.figure(figsize=(10, 6))
plt.bar(np.arange(size), column_sums, color='salmon')
plt.xticks(ticks=np.arange(size), labels=reversed(sorted_benchmarks), rotation=90)
plt.ylabel('Value')
plt.title('Column Sum of Contribution Matrix $C$')
