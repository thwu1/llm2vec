...
def load_data(pwd):
    train_x = torch.tensor(torch.load(f'{pwd}/data_new/pc_new_train_x.pth')).float()
    train_y = torch.tensor(torch.load(f'{pwd}/data_new/pc_new_train_y.pth')).float()
...
class Trainer:  # batch-wise autoregressively input k question and get (k+1)_th questions' answer
    def __init__(self, encoder, decoder, sample_length, train_dataloader, train_val_dataloader, 
        ...
        NUM_TESTS = 5
        TEST_SEEDS = [random.randint(0, 1000) for i in range(NUM_TESTS)]
        self.test_num = NUM_TESTS
        self.test_seeds = TEST_SEEDS
        print(self.test_seeds)
    ...
    def evaluate_max_context(self, train_val_dataloader, test_dataloader):
        ...
        for k in range(self.test_num):
            ...
            # Perform Eval on the whole test_dataloader
...
if __name__ == "__main__":
    ...
    Z_DIM = 256 # Z_DIM choices: 32,64,96,128
    USE_KL = True
